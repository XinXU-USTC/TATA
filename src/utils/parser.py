...
def parse_ground_truth(example: Dict[str, Any], data_name):
    ...
    if data_name in ["math", 'ocw']:
        ...
        gt_cot, gt_ans = None, example['target']
    elif data_name in ["collegemath", "mathematics", "gaokaobench", "gaokao2023"]:
        gt_cot, gt_ans = None, example['answer']
    elif data_name == "theoremqa":
        gt_cot, gt_ans = None, example['Answer']
...
sys.path.append(os.path.join(os.path.dirname(os.path.dirname(current_directory)), "dart-math"))
...
def run_execute(executor, result, prompt_type, execute=False):
    ...
    if "program_only" in prompt_type:
        ...
        #try:
        #    prediction = evaluator.extract_ans(result)
        #except:
        #    prediction = extract_answer(result)
        prediction = extract_answer(result)

    try:
        prediction = evaluator.norm_ans_str(prediction)
    except:
