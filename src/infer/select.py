...
from utils.utils import set_seed, load_jsonl, save_jsonl, construct_prompt_new
...
def parse_args():
    ...
    parser.add_argument("--data_name", default="select", type=str) # data name should be which SFT data we want to select.
    parser.add_argument("--test_data", default="anchor", type=str) # data set to test (anchor set) which is fixed 
    parser.add_argument("--data_dir", default="./data", type=str)
    parser.add_argument("--model_name_or_path", default="gpt-4", type=str)
    parser.add_argument("--output_dir", default="./output", type=str)
    parser.add_argument("--split", default="dm-hard-494k", type=str) # split is used to determine SFT split to select
    parser.add_argument("--prompt_type", default="cot", type=str) # cot or pot
    parser.add_argument("--test_split", default="dm-hard_kmeans100", type=str) # split to test random or kmeans
...
def prepare_data(args):
    ...
    # load SFT data to select (q, cot, pot)
    ...
    for item in examples:
        new_examples.append({"query": item["query"],
                            "response": item[args.prompt_type],
                            "idx": item['idx'],
                            "cot": item['cot'],
                            "pot": item['pot']})
    ...
    test_examples = load_data(args.test_data, args.test_split, args.data_dir)

    # get out_file name
    dt_string = datetime.now().strftime("%m-%d_%H-%M")
    model_name = "/".join(args.model_name_or_path.split("/")[-2:])
    out_file_prefix = f'seed{args.seed}_t{args.temperature}'
    out_dir = f'{args.output_dir}/{model_name}/{args.test_split}/{args.split}/{args.prompt_type}'
    out_file = f'{out_dir}/{out_file_prefix}_s{args.start}_e{args.end}.jsonl'
    os.makedirs(f'{out_dir}', exist_ok=True)
    
    return examples, test_examples, out_file


def main(args):
    processed_samples, examples, out_file = prepare_data(args)
    ...
    for example in tqdm(examples, total=len(examples)):
        ...
        # TODO: parse ground truth
        gt_cot, gt_ans = parse_ground_truth(example, args.data_name)

        # TODO: construct prompt according to cot or pot alpaca format is ok
        for ddd in processed_samples:
            full_prompt = construct_prompt_new(args, example, ddd)

            sample = {'sft_id': ddd['idx'], 'query': ddd['query'], 'cot_response': ddd['cot'], 'pot_response': ddd['pot'], 'test_idx': idx, 'test_question': example['question'], 'gt_cot': gt_cot, 'gt': gt_ans, 'prompt': full_prompt}
    ...
    codes = [prompt.strip() for _, prompt in end_prompts]
